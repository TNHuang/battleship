(function() {
    ...
    $(".fleet-container").on("click", "li", function(event){
      ...
      if (side === this.currentSide) {
        ...
        if (this.currentSide === "my"){
          ...
          if (this.board.enemyShips.indexOf(coord) === -1) {
            console.log("coord: ", coord[0], ",", coord[1]);
            console.log(this.board.enemyShips.indexOf(coord));
            this.board.myVolley.push(coord);
          } else {
            this.board.myMiss.push(coord)
          }

        }else {
          this.currentSide = "my"

          if (this.board.myShips.indexOf(coord) !== -1) {
            this.board.enemyVolley.push(coord);
          } else {
            this.board.enemyMiss.push(coord)
          }
        }
      }
      this.updateBoard();
  ...
  View.prototype.updateBoard = function(){
    ...
    var myMiss = this.board.myMiss;
    var enemyMiss = this.board.enemyMiss;
    ...
    myVolley.forEach (function(coord){
      $(".enemy-ocean ul.col:nth-child(" + (coord[0] + 1) + ") li:nth-child(" + (coord[1] + 1) + ")").addClass("volley");
    });

    enemyVolley.forEach (function(coord){
      $(".my-ocean ul.col:nth-child(" + (coord[0] + 1) + ") li:nth-child(" + (coord[1] + 1) + ")").addClass("volley");
    });

    myMiss.forEach (function(coord){
      $(".enemy-ocean ul.col:nth-child(" + (coord[0] + 1) + ") li:nth-child(" + (coord[1] + 1) + ")").addClass("miss");
    });

    enemyMiss.forEach (function(coord){
      $(".my-ocean ul.col:nth-child(" + (coord[0] + 1) + ") li:nth-child(" + (coord[1] + 1) + ")").addClass("miss");
    });
  };
})();
